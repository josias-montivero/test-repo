...
public class BasicProgression : BattleBitModule
{
    ...
    public override async Task OnSavePlayerStats(ulong steamID, PlayerStats stats)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            try
            {
                ...
                File.WriteAllText(getPlayerFileNameJSON(steamID), JsonSerializer.Serialize(stats));
                return;
            }
            catch (Exception ex)
            {
    ...
    private string getPlayerFileNameJSON(ulong steamId)
    {
        return Path.Combine(dataDir, $"{steamId}.json");
    }

    private string getPlayerFileNameJSON2(ulong steamId)
    {
        return Path.Combine(dataDir, $"{steamId}_player.json");
    }
}
